...
public class ReadExcel {
	 ...
	 * 设定row开始读取的位置，默认为0
	 */
	private int rowStartReadPos = READ_START_POS;
 
	/**
	 * 设定row结束读取的位置，默认为0，用负数来表示倒数第N行,倒数第一行(-N)
	 */
	private int rowEndReadPos = READ_END_POS;
	
	/**
	 * 设定cell开始读取的位置，默认为0
	 */
	private int cellStartReadPos = 0;
	
	/**
	 * 设定cell结束读取的位置，默认为0，用负数来表示倒数第N行,倒数第一行(-N)
	 */
	private int cellEndReadPos = 0;
	...
	public static void main(String[] args) {
		...
		//从第一行开始读取
		eu.setRowStartReadPos(1);
		
		String src_xlspath = "D:\\eduUpload\\a.xls";
		List<List<String>> rowList;
		try {
			rowList = eu.readExcel(src_xlspath);
			for (int i = 0; i < rowList.size(); i++) {
				System.out.print(i);
				for (String s : rowList.get(i)) {
					System.out.print(s+"\t|");
				}
				System.out.println();
			}
	...
	private List<Row> readExcel(Workbook wb) {
		...
		for(int t=startSheetIdx; t<sheetCount+endSheetIdx;t++){
			...
			Row row = null;
			// 循环读取
			for (int i = rowStartReadPos; i <= lastRowNum + rowEndReadPos; i++) {
				row = sheet.getRow(i);
				if (row != null) {
					rowList.add(row);
					out("第"+(i+1)+"行：",false);
	 ...
	 * 
	 */
	private List<List<String>> readExcel4List(Workbook wb) {
		List<List<String>> rowList = new ArrayList<List<String>>();
		...
		for(int t=startSheetIdx; t<sheetCount+endSheetIdx;t++){
			...
			for (int i = rowStartReadPos; i <= lastRowNum + rowEndReadPos; i++) {
				...
				if (row != null) {
					...
					out("第"+(i+1)+"行：",false);
					// 获取每一单元格的值
					for (int j = cellStartReadPos; j < row.getLastCellNum() + cellEndReadPos; j++) {
						String value = getCellValue(row.getCell(j));
						cellList.add(value);
						out(value.equals("")?" ":value + "\t|\t",false);
	...
	private void writeExcel(Workbook wb, List<Row> rowList, String xlsPath) {
		...
		int lastRowNum = isOverWrite ? rowStartReadPos : sheet.getLastRowNum() + 1;
		int t = 0;//记录最新添加的行数
	...
	private int findInExcel(Sheet sheet, Row row) {
		...
		try {
			...
			for (int i = rowStartReadPos; i <= sheet.getLastRowNum() + rowEndReadPos; i++) {
	...
	public int getCellStartReadPos() {
		return cellStartReadPos;
	}

	public void setCellStartReadPos(int cellStartReadPos) {
		this.cellStartReadPos = cellStartReadPos;
	}

	public int getCellEndReadPos() {
		return cellEndReadPos;
	}

	public void setCellEndReadPos(int cellEndReadPos) {
		this.cellEndReadPos = cellEndReadPos;
	}
}
